...
- 엔티티를 유형과 무형으로 분류하는 기준은 **물리적 형태의 존재 여부**
- 유형과 무형에 따른 엔티티 종류
    - 유형  엔티티
    ...
    - 개념 엔티티
        ...
        - 개념적으로 사용되는 엔티티이다.
        - e.g. 거래소 종목, 코스닥 종목, 보혐 상품
    - 사건 엔티티
        - **비즈니스 프로세스를 실행하면서 생성되는 엔티티**
        - e.g. 주문, 체결, 취소주문, 수수료 청구 등
...
![https://s3-us-west-2.amazonaws.com/secure.notion-static.com/0d6d1155-ca29-421a-818e-f20b9ce46898/Untitled.png](https://s3-us-west-2.amazonaws.com/secure.notion-static.com/0d6d1155-ca29-421a-818e-f20b9ce46898/Untitled.png)\
...
- 속성은 더 이상 분이되지 않는 단위로, 업무에 필요한 데이터를 저장할 수 있다
- 인스턴스 구성요소이고 의미적으로 더 이상 분해되지 않는다
...
- 존재 관계는 두 개의 엔티티가 존재 여부의 관계가 있는 것이고
행위 관계는 두 개의 엔티티가 어떤 행위에 의한 관련성이 있는 것
...
- 행위 관계는 엔티티 간에 어떤 행위가 있는 것을 말한다.
- e.g. 증권회사가 계좌를 개설하고 주문을 발주하는 것

## 3. 관계 차수(Cardinality)

### (1) 관계 차수(Relation Cardinality)

- 관계 차수는 두 개의 엔티티 간의 관계에 참여하는 수를 의미한다
...
- 완전 1대1
    ...
    - 반드시 존재
- 선택적 1대1
    - 하나의 엔티티에 관계되는 엔티티의 관계가 하나이거나 없을 수도 있따

② 1대N 관계

- 1대N 관계는 엔티티에 행이 하나 있을 때 다른 엔티티의 값이 여러 개 있는 관계
- e.g. 고객은 여러 개의 계좌를 가질 수 있다

③ M대N 관계

- 두 개의 엔티티가 서로 여러 개의 관계를 가지고 있는 것
- e.g. 한 명의 학생이 여러개의 과목을 수강할 수 있고
반대로 한 개의 과목은 여러 명의 학생이 수행한다 ⇒ M대N
